                    ...
                    <div class="app-container app-theme-white body-tabs-shadow fixed-sidebar fixed-header">
                        ...
                        <!-- Main Content Starts from Here -->
                            <div class="app-main">
                                    ...
                                    <div class="app-main__outer">
                                        <div class="app-main__inner">
                                            ...
                                            <?php 
                                                ...
                                                if(isset($_POST['submitMarkAsComplete']) && !empty($_POST['submitMarkAsComplete']))
                                                {
                                                    $id = $_POST['submitMarkAsComplete'];
                                                    // $is_int = is_int($id) ? "true" : "false";  ASSIGNMENT:-  FIND HOW TO CHECK IF A VARIABLE IS AN INTEGER

                                                    $transactionID = $_POST['submitMarkAsComplete'];

                                                    if(isset($transactionID) && !empty($transactionID))
                                                    {
                                                        $sql = "
                                                                    UPDATE transactions SET transc_status = 'completed' WHERE id = '$transactionID'
                                                                ";
                                                        ...
                                                        if($checks === true OR $count>0)
                                                        {
                                                            $alertMsg =     '<div class="col-sm-12 alert alert-success alert-dismissable">'.
                                                                                '<button aria-hidden="true" data-dismiss="alert" class="close" type="button">×</button>'.
                                                                                '<h6><i class="icon fa pe-7s-check"></i> Success marked as completed</h6>'.
                                                                            '</div>';
                                                        }
                                                        else
                                                        {
                                                            $alertMsg =  '<div class="col-sm-12 alert alert-danger alert-dismissable">'.
                                                                            '<button aria-hidden="true" data-dismiss="alert" class="close" type="button">×</button>'.
                                                                            '<h6><i class="icon fa pe-7s-attention"></i> failed, because the transaction status was already completed</h6>'.
                                                                        '</div>';
                                                        }
                                                    }
                                                    else
                                                ...
                                                {   
                                                    ...
                                                    $start = ($page - 1) * $limit;
                                                    $sql = "
                                                                SELECT transactions.*, users.username, users.full_name AS fullname FROM transactions, users WHERE 
                                                                (transactions.reference = '$input' OR username = '$input' OR full_name LIKE '%$input%' OR amount LIKE '%$input' OR id LIKE '%$input' OR transc_time LIKE '%$input%' OR 
                                                                day LIKE '%$input%' OR month LIKE '%$input' OR year LIKE '%$input' OR purpose LIKE '%$input') 
                                                                && transactions.user_id = users.user_id && transc_status = 'completed' ORDER BY transactions.id DESC LIMIT $start, $limit
                                                            ";
                                                    ...
                                                    $result1 = $conn->prepare("
                                                                                SELECT count(transactions.id) AS id FROM transactions, users WHERE 
                                                                                (transactions.reference = '$input' OR username = '$input' OR full_name LIKE '%$input%' OR amount LIKE '%$input' OR id LIKE '%$input' OR transc_time LIKE '%$input%' OR 
                                                                                day LIKE '%$input%' OR month LIKE '%$input' OR year LIKE '%$input' OR purpose LIKE '%$input' OR transc_status LIKE '%$input') 
                                                                                && transactions.user_id = users.user_id && transc_status = 'completed'
                                                                            ");
                                                ...
                                                {   
                                                    ...
                                                    $sql = "
                                                                SELECT transactions.*, users.username, users.full_name AS fullname FROM transactions, users WHERE 
                                                                transactions.user_id = users.user_id ORDER BY transactions.id DESC LIMIT $start, $limit
                                                            ";
                                            ...
                                            ?>
                                                ...
                                                <?php include_once 'transactionscontent.php'; ?>
                                            </div>
                                    ...
                                    <div class="modal fade bd-example-modal-lg mt-5" id="markAsCompleteModal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" data-backdrop="false">
                                        <div class="modal-dialog">
                                            ...
                                            <div class="modal-body">
                                                <div class="position-relative">
                                                    <p>
                                                        <h6 class="text-center">Are you sure you want to mark this transaction as completed?</h6>
                                                    </p>
                                                </div>
                                            </div>
                                            <div class="modal-footer">
                                                <form method="POST">
                                                    ...
                                                    <button name="submitMarkAsComplete" type="submit" value="" class="mr-5 btn-transition btn btn-outline-danger finalMarkAsComplete">
                                                        Yes, Proceed
                                                    </button>
                                                </form>
                                            </div>
                    ...
                    <script type="text/javascript">
                        ...
                        // $('.flash_msg').delay(250).fadeOut('slow');
                            ...
                            $('.markAsComplete').click(function (){
                                ...
                                $('.finalMarkAsComplete').attr("value", name);
